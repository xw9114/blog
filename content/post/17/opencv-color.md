...
下面给出一个基于 OpenCV 的颜色识别示例：输入 BGR 图像，转换到 HSV 空间后完成阈值分割、形态学净化、中央 ROI 约束下的最大目标提取，并把目标质心映射成控制系统可直接使用的归一化偏差。代码重点不在“调用几个 API”，而在 **如何把像素集合收敛成稳定、可控、可映射的几何输出**。
                            ...
                            cv::Mat *debug_mask)
{
    ...
    edge_margin_ratio = ClampFloat(edge_margin_ratio, 0.0f, 0.45f);
    ...
    min_center_x = static_cast<float>(bgr_frame.cols - 1) * edge_margin_ratio;
    max_center_x = static_cast<float>(bgr_frame.cols - 1) * (1.0f - edge_margin_ratio);
    min_center_y = static_cast<float>(bgr_frame.rows - 1) * edge_margin_ratio;
    max_center_y = static_cast<float>(bgr_frame.rows - 1) * (1.0f - edge_margin_ratio);
    ...
    for (std::size_t i = 0; i < contours.size(); ++i)
    {
        ...
        candidate_moments = cv::moments(contours[i]);
        if (candidate_moments.m00 <= 1e-5)
        {
            continue;
        }

        center_x = static_cast<float>(candidate_moments.m10 / candidate_moments.m00);
        center_y = static_cast<float>(candidate_moments.m01 / candidate_moments.m00);

        /* 只保留中央区域内的质心：
         * min_cx = (W - 1) * edge_margin_ratio
         * max_cx = (W - 1) * (1 - edge_margin_ratio)
         * min_cy = (H - 1) * edge_margin_ratio
         * max_cy = (H - 1) * (1 - edge_margin_ratio)
         *
         * 若目标中心落在边缘留白区域内，则直接忽略。
         */
        if ((center_x < min_center_x) || (center_x > max_center_x)
